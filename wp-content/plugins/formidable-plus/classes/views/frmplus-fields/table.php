...
<table style="border:none !important;" id="frm-table-<?php echo $field['id']; ?>" class="frm-gde-steps frm-table<?php if (count($classes = apply_filters('frm_table_classes',array(),$field['id']))) echo ' '.join(' ',$classes); ?>">	
<?php 
// NEIGHBORHOW MOD
...
<a class="frmplus-add-row" id="frmplus-add-row-<?php echo $field['id']; ?>" href="javascript:add_row(<?php echo $field['id']; ?>)"><?php echo apply_filters('frmplus_text_add_row','Add another Step',$field); ?> <img style="vertical-align:middle" src="<?php echo FRM_IMAGES_URL ?>/duplicate.png" alt="Add another step" title="Add another Step" border="0"></a>
