...
// added div wrapper, required, help block, and validation	
	...
	<input autocomplete="off" name="pass1" id="pass1<?php $template->the_instance(); ?>" class="input" size="20" value="" type="password" tabindex="30" required pattern="^[a-zA-Z0-9-_!?$%^\&)\.]{7,16}$" />
	<span class="help-block">Your password should be at least 7 characters long. To make it stronger, use upper and lower case letters, numbers and symbols like ! " ? $ % ^ &amp; ).</span>
	</div>
	<div class="form-item"><label for="pass2<?php $template->the_instance(); ?>"><?php _e( 'Password confirmation', 'theme-my-login' );?></label>
	<input autocomplete="off" name="pass2" id="pass2<?php $template->the_instance(); ?>" class="input" size="20" value="" type="password" tabindex="30" required pattern="^[a-zA-Z0-9-_!?$%^\&)\.]{7,16}$" />
